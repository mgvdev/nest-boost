...
| Tool | Description |
| --- | --- |
| `application_info` | Bun/Node/Nest versions, detected ecosystem packages with versions, and module/controller/provider/route counts |
...
- **Baseline** — `nestjs-development`, `nestjs-docs` (authoritative lookups against the
  ...
  `testing-jest`
- **Package-gated** — e.g. `typeorm-development`, `graphql-development` (only when detected)
...
install options:
  ...
  --entry <path>        Root module file (default: src/app.module.ts)
  --module <name>       Root module export (default: AppModule)
