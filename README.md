...
A library can ship its own skill and have nest-boost install it automatically when the
library is a dependency (like Laravel Boost's third-party package skills). Declare it in the
...
the conventional roots `nest-boost/skills/`, `skills/`, and `skill/`. Discovered package
skills are installed by `install`/`update`; a local skill of the same name still wins.

Priority (low → high): **packaged** → **package-bundled** → **local**.
